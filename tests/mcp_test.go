...
func (s *YumSuite) TestListTools() {
	...
	s.Len(result.Tools, 9)
	toolsName := []string{}
	for _, tool := range result.Tools {
		toolsName = append(toolsName, tool.Name)
	}

	s.Equal([]string{"accesstoken", "audit_video", "authenticate", "channels", "get_video", "refreshtoken", "update_video", "update_video_metadata", "upload_video"}, toolsName)
}
